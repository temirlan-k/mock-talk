...
const BASE_DEV_URL = 'http://0.0.0.0:8080';
...
export function HeyGen() {
    ...
    useEffect(() => {
        const ws = new WebSocket(`${BASE_DEV_URL.replace('http', 'ws')}/ws/chat`);
        ...
        ws.onmessage = (event) => {
            ...
            // Log the full WebSocket message to verify its structure
            ...
            // Call speakText with the response
            if (data?.sessionId) {
                speakText(response, data.sessionId);
            }
        };
